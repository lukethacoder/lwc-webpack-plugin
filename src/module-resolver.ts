...
const lwcResolver = require('@lukethacoder/lwc-module-resolver')
  ...
  resolveFile(req: any, ctx: any, cb: any) {
    ...
    this.fs.stat(resourcePath, (err: { code: string } | null) => {
      if (err !== null && err.code === 'ENOENT') {
        ...
        if (isCSS && !isImplicitCssImport(resourcePath, req.context.issuer)) {
          // warning for a missing non-implicit CSS import
          console.warn(
            `The imported CSS file ${resourcePath} does not exist: Importing it as undefined.`
          )
        }
        if (isCSS || isImplicitHTMLImport(resourcePath, req.context.issuer)) {
          return cb(null, {
            path: EMPTY_STYLE,
            query,
            file: true,
